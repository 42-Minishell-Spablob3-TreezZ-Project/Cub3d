...
/*   Updated: 2026/07/14 12:27:02 by joapedro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_game(t_map *map)
{
	...
	map->data.mlx_win = mlx_new_window(map->data.mlx, \
WIDTH, HEIGHT, "cub3D");
	map->data.img.mlx_img = mlx_new_image(map->data.mlx, WIDTH, HEIGHT);
	map->data.img.addr = mlx_get_data_addr(map->data.img.mlx_img, \
&map->data.img.bpp, &map->data.img.line_len, &map->data.img.endian);
}
